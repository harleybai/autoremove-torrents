...
class uTorrent(object):
    def __init__(self, host):
        ...
        # Floating decimal in seconds,eg 1459996086.7115328
    ...
    def login(self, username, password):
        ...
        if request.status_code == 200:
            ...
            raise RemoteFailure('The server response %d.' % request.status_code)

    # Get uTorrent Version
    def version(self):
        if self._version == '':  # Call torrents_list() to get the version
            self.torrents_list()
        return 'uTorrent (build %s)' % str(self._version)
    ...
    def torrents_list(self):
        ...
        if request.status_code != 200:  # Error
            raise RemoteFailure('The server response %s.' % request.text)
        result = request.json()
